...
## 11. 라이브러리 갱신 — cweijan/vscode-office (MIT) 참고

VS Code 확장 `cweijan/vscode-office`(**MIT, 완전 오픈소스**)의 렌더러 선택을 참고해 P2/P3 라이브러리를 확정한다. 확장 자체를 이식하지 않는다(VS Code 웹뷰 API 결합·불필요한 무거운 의존성) — **라이브러리 선택만 채택**하고 우리 Tauri+React 아키텍처로 직접 구현한다.

| 포맷 | 채택 라이브러리 | 비고 |
|------|----------------|------|
| PDF | `pdfjs-dist` | P1과 동일 |
| Excel(.xlsx) | `xlsx`(SheetJS 파싱) + **`x-data-spreadsheet`**(그리드 렌더) | 기존 "HTML 표"안 대체 — 실제 스프레드시트 UI |
| Word(.docx) | **`@eigenpal/docx-editor-react`**(React 컴포넌트) 우선, 여의치 않으면 `docx-preview` | React 스택에 직접 결합 |
| PowerPoint(.pptx) | **`pptxviewjs`** | 기존 "베스트-에포트/폴백"에서 정식 뷰어로 승격(폴백은 유지) |

**착수 시 due-diligence(P2/P3)**: 각 라이브러리의 라이선스(MIT/Apache 등)·유지보수 상태·번들 크기·Vite 호환(동적 import/worker)을 확인 후 확정한다. 부적합하면 대체(docx-preview 등)로 폴백.

**§9 단계 갱신**: P2 = Excel(`x-data-spreadsheet`) + Word(`@eigenpal/docx-editor-react`); P3 = PowerPoint(`pptxviewjs`) + 미지원 폴백. **P1은 영향 없음**(이미지·PDF·MD, pdf.js 동일).
