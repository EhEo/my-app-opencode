...
## KI-2 — `XlsxViewer`의 x-data-spreadsheet 그리드가 파일 열 때마다 `window` 리스너 누수

- **상태**: 열림 / **보류** (경미·바운드됨. 크래시 없음)
- **심각도**: 낮음 — `x-data-spreadsheet`(라이브러리) `Sheet` 생성자가 `window`에 `resize`·`click` 리스너를 익명 클로저로 등록하지만 destroy()/dispose() API가 없어 제거 불가. xlsx 파일을 열 때마다(파일 전환·재오픈) 그리드 인스턴스와 함께 리스너가 누적된다.
- **재현**: `src/components/viewers/XlsxViewer.tsx`에서 xlsx 파일을 반복해서 열거나 전환할 때마다.

### 완화 조치

언마운트/재실행 시 effect cleanup에서 `host.innerHTML = ""`로 호스트 DOM 서브트리를 비워 캔버스·DOM은 즉시 GC 대상이 되도록 함. 다만 `window`에 걸린 리스너 자체는 업스림에 teardown 훅이 없어 제거되지 않고 남는다 — 클로저가 참조하는 DOM은 detach되어 메모리 영향은 제한적이나, 리스너 객체 자체는 프로세스 종료 전까지 누적된다.

### 수정 후보

- 업스트림(`x-data-spreadsheet`)에 destroy()/dispose() API 추가를 요청하거나 fork.
- 대안 라이브러리로 교체 (범위 밖, 별도 작업 필요).
